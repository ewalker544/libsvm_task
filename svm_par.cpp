...
void send_problem(const svm_problem *prob, int rank) 
{
	...
	for (int i = 0; i < prob->l; ++i) 
	{
		auto it = SVM_NODE_MAP.find(prob->x[i]);
		if (it == SVM_NODE_MAP.end()) {
			// panic!
			std::cerr << "could not find index address x[" << i << "] = " << prob->x[i] << std::endl;
...
svm_problem *recv_problem(svm_node **x, int rank)
{
	MPI::Status status;
	svm_problem *prob = Malloc(svm_problem, 1);

	MPI::COMM_WORLD.Recv(&prob->l, 1, MPI_INT, rank, 0, status);
	...
	int *buffer = new int[prob->l];
	MPI::COMM_WORLD.Recv(buffer, prob->l, MPI_INT, rank, 0, status);
	...
	MPI::COMM_WORLD.Recv(prob->y, prob->l, MPI_DOUBLE, rank, 0, status);
...
svm_parameter *recv_param(int rank) 
{
	...
	// Receive the ints 
    int intbuf[6];	
	MPI::COMM_WORLD.Recv(intbuf, 6, MPI_INT, rank, 0, status);

	param->svm_type 	= intbuf[0];
	param->kernel_type 	= intbuf[1];
	param->degree 		= intbuf[2];
	param->nr_weight 	= intbuf[3];
	param->shrinking 	= intbuf[4];
	param->probability 	= intbuf[5];

	// send the doubles
	double dbuf[7];
	MPI::COMM_WORLD.Recv(dbuf, 7, MPI_DOUBLE, rank, 0, status);

	param->gamma 		= dbuf[0];
	param->coef0 		= dbuf[1];
	param->cache_size 	= dbuf[2];
	param->eps 			= dbuf[3];
	param->C 			= dbuf[4];
	param->nu 			= dbuf[5];
	param->p 			= dbuf[6];
	...
	if (param->nr_weight > 0) {
		...
		MPI::COMM_WORLD.Recv(param->weight_label, param->nr_weight, MPI_INT, rank, 0, status);

		param->weight = Malloc(double, param->nr_weight);
		MPI::COMM_WORLD.Recv(param->weight, param->nr_weight, MPI_DOUBLE, rank, 0, status);
	}
...
int do_svm_train_one(svm_node **x, int rank)
{
	...
	svm_problem * prob = recv_problem(x, rank);
	if (prob == NULL)
		return -1;

	svm_parameter * param= recv_param(rank);
	if (param == NULL)
		return -1;

	double costs[2];
	MPI::Status status;
	MPI::COMM_WORLD.Recv(costs, 2, MPI_DOUBLE, rank, 0, status);

	decision_function f = svm_train_one(prob, param, costs[0], costs[1]);

	// Send results
	MPI::COMM_WORLD.Send(f.alpha, prob->l, MPI_DOUBLE, rank, 0);
	MPI::COMM_WORLD.Send(&f.rho, 1, MPI_DOUBLE, rank, 0);
	...
	else
	{
		...
		for(int i=0;i<nr_class;i++)
		{
			for(int j=i+1;j<nr_class;j++)
			{
				...
				// Push work item into our vector of work_items
				work_items.push_back(std::make_pair(func, nullptr));
